...
export default function LoginPage() {
  ...
  const onDevSubmit = (e: FormEvent) => {
    submitAuthForm(e, handleDevLogin, setError, setSubmitting);
  };
  ...
  return (
    ...
    >
      ...
      {config?.devLoginEnabled && (
        <>
          ...
          <form onSubmit={onDevSubmit} className="space-y-3">
            <p className="text-xs text-muted">Development only — email sign-in without a password.</p>
      ...
      {config && !config.googleOauthConfigured && !config.passwordAuthEnabled && !config.devLoginEnabled && (
