...
const envSchema = z.object({
  ...
  // Better Auth (identity). Secret must be long + random (openssl rand -hex 32).
  BETTER_AUTH_SECRET: z.string().min(32, 'BETTER_AUTH_SECRET must be at least 32 characters'),
  BETTER_AUTH_URL: z.string().optional().default(''),

  // Legacy JWT vars — the JWT auth system was replaced by Better Auth. Kept
  // optional for backwards compatibility; no longer used for authentication.
...
// Better Auth's public base URL. Defaults to the backend's own public URL,
// since the auth handler is mounted on the Express API (at /api/auth).
const betterAuthUrl = raw.BETTER_AUTH_URL.trim().length > 0 ? normalizeUrl(raw.BETTER_AUTH_URL) : appUrl;
